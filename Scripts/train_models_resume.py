...
NOTE:
This ConvLSTM model was taking too much time to be trained as per the previous set epochs,
which I had set to 50, and i didn't have the time it required to complete its training cause the deadline was approaching,
So i paused the training when the folds(AP01 and AP02) had completed, and then made this script to train
the ConvLSTM with the reduced epochs(15) for the remaining folds(AP03,04,05)
'''
...
def leave_one_participant_out_cv(model_class, model_name, dataset_path, results_dir, fine_tune_epochs=15):
    print(f"\n{'='*70}")
    print(f"Resumed Training {model_name} - (skip folds 1,2, reduced epochs after)")
    print(f"{'='*70}\n")
    ...
    for fold_idx, test_participant in enumerate(participants):
        if fold_idx < 2:  # Skip first 2 folds
            print(f"\n{'─'*70}")
            print(f"SKIPPING FOLD {fold_idx + 1}/{len(participants)} - {test_participant} (already completed)")
            print(f"{'─'*70}")
            continue
        print(f"\n{'─'*70}")
        ...
        print("Training with reduced epochs...")
        for epoch in range(fine_tune_epochs):
            train_loss, train_acc = train_epoch(model, train_loader, criterion, optimizer, device)
            if (epoch + 1) % 5 == 0 or epoch == fine_tune_epochs - 1:
                print(f"  Epoch {epoch+1}/{fine_tune_epochs} - Loss: {train_loss:.4f}, Acc: {train_acc:.2f}%")
        print("Evaluating...")
    ...
    print(f"\n{'='*70}")
    print(f"OVERALL RESULTS - {model_name} (Resumed/Fast CV)")
    print(f"{'='*70}\n")
    # (Metrics aggregation not repeated here; see original script for full result printing.)

def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('--dataset', required=True, help='Path to dataset parquet file')
    parser.add_argument('--model', required=True, choices=['cnn', 'convlstm'], help='Model architecture')
    parser.add_argument('--results_dir', default='Results', help='Directory to save results')
    parser.add_argument('--epochs', type=int, default=15, help='Reduced number of epochs for fast training')
    ...
    leave_one_participant_out_cv(model_class, model_name, args.dataset, results_dir, fine_tune_epochs=args.epochs)
    print("\n" + "="*70)
    print("Hybrid training: First 2 folds were trained with 50 epochs; remaining folds with reduced epochs for deadline.")
